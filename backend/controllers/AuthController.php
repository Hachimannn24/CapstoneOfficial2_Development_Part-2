...
class AuthController
{
    ...
    public static function getStaff(): void
    {
        try {
            $db   = Database::getConnection();
            $stmt = $db->query('SELECT id, name, email, role, branch, is_active, backup_email, mfa_enabled, google_id, google_email, is_online, last_active, created_at, updated_at FROM users ORDER BY created_at ASC');
            $staff = $stmt->fetchAll();
    ...
    public static function googleLogin(): void
    {
        ...
        try {
            $db   = Database::getConnection();
            $stmt = $db->prepare('SELECT * FROM users WHERE google_email = ? OR email = ? LIMIT 1');
